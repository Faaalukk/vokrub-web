import type { TopWord } from "../../lib/api";
...
export default function MostStoredWords({ topWords }: { topWords?: TopWord[] }) {
  const words = topWords && topWords.length > 0 ? topWords : FALLBACK;
        ...
        {words.map((entry, index) => (
          <div key={entry.word} className="flex flex-col gap-1">
            <div className="flex justify-between text-sm">
              ...
              <span className="text-gray-500">{entry.count.toLocaleString()}</span>
            </div>
